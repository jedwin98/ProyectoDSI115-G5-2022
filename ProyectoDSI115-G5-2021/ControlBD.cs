    ...
    class ControlBD 
    {
      ...
      //  List<GestionClientes.Cliente> clientes = new List<GestionClientes.Cliente>();
        ...
        public List<EmpleadoItem> ConsultarEmpleadosLista()
        {
            ...
            string comandoString = "SELECT e.cod_empleado,e.nombre_empleado,e.apellido_empleado,a.nombre_area as narea,c.nombre_cargo as ncargo FROM ((empleado AS e INNER JOIN cargo AS c ON e.cod_cargo = c.cod_cargo) INNER JOIN area AS a ON e.cod_area = a.cod_area) INNER JOIN usuario AS u ON e.cod_empleado = u.cod_empleado WHERE u.cod_empleado IS NULL AND estado_empleado='Activo' AND NOT(e.cod_cargo = 'ST' AND e.cod_area = 'L')";
